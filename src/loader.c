...
int load_random(Simulation * simulation, const char * path){
    *simulation = (Simulation){0};
    ...
    if(!cJSON_IsNumber(scale)){
        printf("Error: count must me a number\n");
        return 1;
    }
    cJSON *maxdist = cJSON_GetObjectItemCaseSensitive(json, "maxdist");
    if(!cJSON_IsNumber(scale)){
        printf("Error: maxdist must me a number\n");
        return 1;
    }
    cJSON *radmul = cJSON_GetObjectItemCaseSensitive(json, "radmul");
    if(!cJSON_IsNumber(scale)){
        printf("Error: radmul must me a number\n");
        return 1;
    }
    cJSON *massmul = cJSON_GetObjectItemCaseSensitive(json, "massmul");
    if(!cJSON_IsNumber(scale)){
        printf("Error: massmul must me a number\n");
        return 1;
    }
    cJSON *minmassmul = cJSON_GetObjectItemCaseSensitive(json, "minmassmul");
    if(!cJSON_IsNumber(scale)){
        printf("Error: minmassmul must me a number\n");
        return 1;
    }
    cJSON *maxmassmul = cJSON_GetObjectItemCaseSensitive(json, "maxmassmul");
    if(!cJSON_IsNumber(scale)){
        printf("Error: maxmassmul must me a number\n");
        return 1;
    }
    for(int i = 0; i < count->valueint; i++){
        char name[NAME_LEN];
        sprintf(name, "R-%d", i+1);
        Body body = new_random_body(
            name,
            massmul->valuedouble, minmassmul->valuedouble, maxmassmul->valuedouble,
            radmul->valuedouble, maxdist->valuedouble, scale->valuedouble
        );
        add_simulation_body(simulation, body);
    }
